...
type PeerInitiateConnectionFactory func(peer *db.Peer, hashInfo []byte) (*torrent.PeerConnection, error)
type ListenPeerConnectionFactory func() (*torrent.PeerConnection, error)
...
type ConnectionManager struct {
	ProtocolId                []byte
	InitiateConnectionFactory PeerInitiateConnectionFactory
	ListenConnectionFactory   ListenPeerConnectionFactory
...
func (cm *ConnectionManager) GetOrConnect(peer *db.Peer, infoHash []byte) (*Connection, error) {
	...
	peerConnection, err := cm.InitiateConnectionFactory(peer, infoHash)
	if err != nil {
		slog.Error("Could not make a connection.")
		return nil, err
	}

	// // Make a connection or die trying.
	// address := fmt.Sprintf("%s:%d", peer.IP, peer.Port)
	// netConn, err := net.Dial("tcp", address)
	// if err != nil {
	// 	slog.Error("Error connecting to " + hexPeerProtocolId + ":" + peer.IP + ":" + strconv.Itoa(peer.Port))
	// 	return nil, err
	// }
	...
	// 		return false, err
	// 	},
	// 	SevereConnection: netConn.Close,
	// }
...
func (cm *ConnectionManager) Accept() (*Connection, error) {
	peerConnection, err := cm.ListenConnectionFactory()
	if err != nil {
		return nil, err
	}
